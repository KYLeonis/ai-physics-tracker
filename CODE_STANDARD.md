...
6. **公式注明依据**：数学变换（如标定的 `R(θ)·diag(s,−s)`）注释引用 spec 章节（"data-model.md §6.2"）；不直观的数值技巧必须解释为什么。
...
- Docstring 要求：public API、领域类、复杂算法有；**简单直观的函数不写**——机械生成的冗长 docstring 是噪音。格式 Google 风格、中文或英文均可但同文件内一致。
