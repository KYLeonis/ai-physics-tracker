...
def _inference_process_worker(
    ...
    request: InferenceRequest, adapter_class: type[EngineAdapter],
) -> JsonObject:
    """大批量结果落盘交换，队列只返回小型摘要，避免退出时等待巨量 IPC。"""
    try:
        digest = _sha256(request.model_snapshot)
        if request.model_sha256 is not None and digest != request.model_sha256:
            raise ValueError("Selected training snapshot has changed; retrain or select another run")
        outcome = adapter_class().infer(run_id, queue, cancel_event, request)
...
class InferenceCoordinator:
    ...
    def prepare_inference(
        ...
        *, adapter: EngineAdapter | None = None, config_path: Path | None = None,
        ...
        # 新推理元数据只保存项目内路径；不把跨平台绝对定位扩散到新 run。
        if not config_path.is_relative_to(root) or not snapshot.is_relative_to(root):
            raise ProjectSessionError("Inference requires config and model inside the saved project")
        ...
        run = create_tracking_run(video.video_id, track.track_id, "infer",
            ...
            config=config, model_snapshot=snapshot.relative_to(root).as_posix())
        request = InferenceRequest(config_path, video_path.resolve(), snapshot,
            root / "data" / "engines" / str(run.run_id), track.track_id, timeline,
            run.source_detail, video.frame_count, params,
            shuffle=config["shuffle"], trainingsetindex=config["trainingsetindex"],
            model_sha256=trained.extra_fields.get("model_sha256"))
        run = replace(run, extra_fields={"config_path": config_path.relative_to(root).as_posix()})
        job = _InferenceJob(session, request, root, video, config["timing_detail"],
                            _stamp(request.video_path), _stamp(config_path), _stamp(snapshot), type(engine))
    ...
    def start_inference(self, session: ProjectSession, run_id: UUID) -> TaskHandle:
        ...
        try:
            ...
            handle = self._runner.start_task(run_id, _inference_process_worker, job.request, job.adapter_class)
    ...
    def _apply_result(self, job: _InferenceJob, run: TrackingRun, payload: JsonObject) -> None:
        ...
        completed = replace(mark_run_completed(run), engine_version=str(payload["engine_version"]), extra_fields=extras)
