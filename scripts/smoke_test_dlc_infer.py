...
from ai_physics_tracker.application.inference_job import InferenceCoordinator
...
from ai_physics_tracker.infrastructure.task_runner import TaskProgress, TaskLog


def wait_for_run(coordinator, session, run_id, handle, log_path, timeout_s=300):
    ...
    try:
        with log_path.open("w", encoding="utf-8") as log:
            while True:
                messages = coordinator.poll_messages(session, run_id)
    ...
    result = next(r for r in session.tracking_runs() if r.run_id == run_id)
...
def main():
    ...
    try:
        ...
        train_run, config = training.prepare_training(session, track.track_id, reader,
            TrainingParams(epochs=1, batch_size=1, device="cpu"))
    finally:
        reader.close()
    handle = training.start_training(session, train_run.run_id, config)
    trained, _ = wait_for_run(training, session, train_run.run_id, handle, root / "training.log")
    ...
    run = inference.prepare_inference(session, trained.run_id,
        InferenceParams(min_confidence=0.0, device="cpu", batch_size=2))
    handle = inference.start_inference(session, run.run_id)
    completed, progress = wait_for_run(inference, session, run.run_id, handle, root / "inference.log")
