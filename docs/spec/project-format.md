...
- `data/engines/dlc/dlc_<track_id前8位>/`：既有训练工作目录（config、训练集与 DLC 模型目录），训练复用该目录。
- `data/engines/<完整infer_run_id>/`：每次推理独占的新目录，保留原始 HDF5/CSV、DLC 元数据和经校验的内部交换文件。目录名使用 UUID，不使用视频名或用户输入。
